  ...
  <view class="page">
    ...
    <view class="section-pad">
      <view class="stats-grid">
        <view class="st-card"><text class="st-num blue">{{ stats.students }}</text><text class="st-lbl">负责学生</text></view>
        <view class="st-card"><text class="st-num green">{{ stats.classes }}</text><text class="st-lbl">管理班级</text></view>
        <view class="st-card"><text class="st-num purple">{{ stats.plans }}</text><text class="st-lbl">帮扶计划</text></view>
      </view>
    </view>
    ...
    <view class="section">
      <view class="card">
        ...
        <view class="info-row">
          ...
          <text class="ir-val">{{ name }}</text>
        </view>
        <view class="ir-div"></view>
        <view class="info-row">
          <text class="ir-label">工号</text>
          <text class="ir-val">{{ info.teacherId || '--' }}</text>
        </view>
        <view class="ir-div"></view>
        <view class="info-row">
          <text class="ir-label">学院</text>
          <text class="ir-val">{{ info.college || '--' }}</text>
        </view>
        <view class="ir-div"></view>
        <view class="info-row">
          <text class="ir-label">管理班级</text>
          <text class="ir-val">{{ info.classes || '--' }}</text>
        </view>
      </view>
    </view>
    ...
    <view class="section">
      <view class="card">
        <view class="action-row" @tap="modifyPassword">
          <text class="ar-label">🔒 修改密码</text>
          <text class="ar-arrow">›</text>
        </view>
        <view class="ar-div"></view>
        <view class="action-row" @tap="clearCache">
          <text class="ar-label">🗑 清除缓存</text>
          <text class="ar-arrow">›</text>
        </view>
      </view>
    </view>
    ...
    <counselor-tab-bar current="pages/counselor/profile" />
  </view>
</template>
...
export default {
  data() {
    return {
      name: '辅导员',
      stats: { students: 0, classes: 0, plans: 0 },
      info: { teacherId: '', college: '', classes: '' }
    }
  },
  ...
  methods: {
    async loadData() {
      try {
        ...
        this.name = uni.getStorageSync('userName') || '辅导员'
        const dash = await counselorAPI.getDashboard(uid)
        if (dash) {
          this.stats.students = dash.totalStudents || 0
          this.stats.classes = dash.totalClasses || 0
          this.stats.plans = dash.assistancePlans || dash.totalAssistance || 0
        }
      } catch (e) {}
    },
    modifyPassword() {
      uni.showModal({
        title: '修改密码', editable: true, placeholderText: '输入新密码',
        success: (r) => {
          if (r.confirm && r.content) uni.showToast({ title: '密码修改成功', icon: 'success' })
        }
      })
    },
    clearCache() {
      uni.showModal({
        title: '确认清除', content: '清除本地缓存数据？',
