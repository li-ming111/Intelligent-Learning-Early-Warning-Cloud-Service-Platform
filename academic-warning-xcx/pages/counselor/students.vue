<template>
  <view class="page">
      ...
      <view v-for="s in filtered" :key="s.id || s.studentId" class="card">
        ...
        <view class="info">
          <view class="name-row">
            ...
            <text class="tag" :class="(s.warnLevel || 0) >= 3 ? 'tag-red' : (s.warnLevel || 0) >= 2 ? 'tag-yellow' : 'tag-green'">
              {{ (s.warnLevel || 0) >= 3 ? '预警' : (s.warnLevel || 0) >= 2 ? '关注' : '正常' }}
            </text>
          </view>
          <text class="meta">学号: {{ s.studentId || s.studentNo }}</text>
          <text class="meta">班级: {{ s.className || '--' }} · {{ s.major || '--' }}</text>
        </view>
        <view class="actions">
          <text class="act-link" @tap="viewDetail(s)">详情</text>
          <text class="act-link primary" @tap="createPlan(s)">帮扶</text>
        </view>
      </view>
    </scroll-view>
    ...
    <counselor-tab-bar current="pages/counselor/students" />
  </view>
</template>
...
export default {
  data() { return { keyword: '', classIdx: 0, classes: [], students: [] } },
  computed: {
  ...
  methods: {
    async loadData() {
      try {
        ...
        const [sRes, cRes] = await Promise.all([counselorAPI.getStudents(uid), counselorAPI.getMyClasses(uid).catch(() => [])])
    ...
    applyFilter() {},
    avGrad(id) {
      const cs = ['#7c3aed,#6d28d9','#2563eb,#1d4ed8','#16a34a,#15803d','#f59e0b,#d97706','#ef4444,#dc2626']
      return `linear-gradient(135deg,${cs[(id||1) % cs.length]})`
    },
    viewDetail(s) {
      uni.showModal({
        title: s.name || s.studentName,
        content: `学号: ${s.studentId}\n班级: ${s.className}\n专业: ${s.major || '--'}\n成绩: ${s.avgScore || '--'}`,
        showCancel: false
      })
    },
    createPlan(s) {
...
.act-link { font-size: 24rpx; color: #64748b; font-weight: 500; padding: 8rpx 16rpx; text-align: center; }
.act-link.primary { color: #2563eb; }
