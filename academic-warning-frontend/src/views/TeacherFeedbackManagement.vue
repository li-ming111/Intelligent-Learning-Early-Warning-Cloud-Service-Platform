<template>
  <div class="tfb-page">
    ...
    <div class="tfb-tool">
      ...
      <el-select v-model="filter" placeholder="全部分类" clearable style="width:150px">
        <el-option v-for="c in categories" :key="c" :label="c" :value="c" />
      </el-select>
    </div>

    <div class="tfb-list" v-if="filtered.length">
      <div v-for="(f,i) in filtered" :key="f.id" class="tfb-card" :class="{pending:f.status==='pending'}" :style="{animationDelay:i*0.06+'s'}">
        <div class="tfc-left">
          <div class="tfc-name">
            <span class="tfc-student">{{ f.studentName }}</span>
            <el-tag size="small">{{ f.category || '其他' }}</el-tag>
            <el-tag size="small" :type="f.status==='pending'?'warning':'success'">{{ f.status==='pending'?'待回复':'已回复' }}</el-tag>
          </div>
          <div class="tfc-content">{{ f.content }}</div>
          <div v-if="f.reply" class="tfc-reply">
            <svg viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2" width="14" height="14"><polyline points="9 10 4 15 9 20"/><path d="M20 4v7a4 4 0 01-4 4H4"/></svg>
            <span>{{ f.reply }}</span>
          </div>
          <div class="tfc-time">{{ fmtDate(f.createdAt) }}</div>
        </div>
        <div class="tfc-right">
          <el-button size="small" type="primary" v-if="f.status==='pending'" @click="openReply(f)">回复</el-button>
          <el-button size="small" v-else @click="viewReply(f)">查看回复</el-button>
        </div>
      </div>
    </div>
    ...
    <el-dialog v-model="replyVisible" title="回复反馈" width="500px" destroy-on-close>
      <div v-if="replyRow">
        <div class="rpl-info">
          <span class="rpl-name">{{ replyRow.studentName }}</span>
          <el-tag size="small">{{ replyRow.category }}</el-tag>
        </div>
        <div class="rpl-content">{{ replyRow.content }}</div>
        <el-divider />
        <el-input v-model="replyText" type="textarea" rows="3" placeholder="输入回复..." />
      </div>
...
const repliedCount = computed(() => feedbacks.value.filter(f => f.status !== 'pending').length)
...
async function loadData() {
  try {
    ...
    const r = await teacherAPI.getFeedbackList(tid)
    if (Array.isArray(r)) feedbacks.value = r.map(f => ({ ...f, status: f.status || (f.reply ? 'replied' : 'pending') }))
...
async function submitReply() {
  ...
  try {
    await teacherAPI.replyToFeedback(replyRow.value.id, { reply_content: replyText.value })
    ElMessage.success('已回复'); replyVisible.value = false; await loadData()
...
.rpl-content { background:#f5f5f5; border-radius:8px; padding:14px; font-size:13px; line-height:1.5; margin-bottom:4px; white-space:pre-wrap; }
